  ...
  async function parseErrorResponse(response) {
    ...
    return (
      ...
      (response.status === 401
        ...
        : response.status === 403
        ? "You do not have permission to manage fund sources."
        : response.status === 419
        ? "Your security token expired. Refresh the page and try again."
        : response.status === 404
        ? "The fund source could not be found."
        : "The request could not be completed.")
    );
  }

  onReady(function () {
    const tableElement = document.getElementById("fund-sources-table");
    if (!tableElement) return;

    const config = window.__gsoFundSources || {};
    const infoElement = document.getElementById("fund-sources-info");
    const searchInput = document.getElementById("fund-sources-search");
    const statusSelect = document.getElementById("fund-sources-status");
    const clusterFilterSelect = document.getElementById("fund-sources-cluster-filter");
    const clearButton = document.getElementById("fund-sources-clear");

    let filters = {
      search: "",
      archived: statusSelect?.value || "active",
      fund_cluster_id: clusterFilterSelect?.value || "",
    };
    let lastTotal = 0;
    ...
    const table = new Tabulator(tableElement, {
      ...
      placeholder: "No fund sources found.",
      pagination: "remote",
      paginationSize: 15,
      ...
      columns: [
        ...
        { title: "Name", field: "name", minWidth: 260 },
        ...
        { title: "Created", field: "created_at_text", width: 200 },
        {
          ...
          formatter: (cell) => {
            ...
            return `
              <div class="hstack flex gap-2 justify-end">
                <button class="ti-btn ti-btn-sm ti-btn-info !rounded-full" type="button" data-action="edit-fund-source" data-row='${escapeHtml(JSON.stringify(row))}'>
    ...
    window.__gsoFundSourcesReload = () => reload(table);

    const applyFilters = debounce(() => {
      filters.search = (searchInput?.value || "").trim();
      filters.archived = (statusSelect?.value || "active").trim();
      filters.fund_cluster_id = (clusterFilterSelect?.value || "").trim();
      reload(table);
    });

    searchInput?.addEventListener("input", applyFilters);
    statusSelect?.addEventListener("change", applyFilters);
    clusterFilterSelect?.addEventListener("change", applyFilters);
    clearButton?.addEventListener("click", () => {
      if (searchInput) searchInput.value = "";
      if (statusSelect) statusSelect.value = "active";
      if (clusterFilterSelect) clusterFilterSelect.value = "";
      filters = { search: "", archived: "active", fund_cluster_id: "" };
      reload(table);
    });
    ...
    document.addEventListener("click", async (event) => {
      ...
      const confirmation = await Swal.fire({
        ...
        title: isRestore ? "Restore fund source?" : "Archive fund source?",
        text: isRestore
          ? "This will restore the fund source."
          : "This will archive the fund source.",
