...
class EloquentAccountableOfficerRepository implements AccountableOfficerRepositoryInterface
{
    public function findOrFail(string $id, bool $withTrashed = false): AccountableOfficer
    {
        $query = AccountableOfficer::query()->with([
    ...
    {
        $query = AccountableOfficer::query()
            ->with([
                'department' => fn ($departmentQuery) => $departmentQuery
                    ->withTrashed()
                    ->select(['id', 'code', 'name', 'short_name']),
