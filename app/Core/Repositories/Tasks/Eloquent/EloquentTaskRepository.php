...
namespace App\Core\Repositories\Tasks\Eloquent;
...
class EloquentTaskRepository implements TaskRepositoryInterface
{
    ...
    {
        return $this->queryInModules($moduleIds)
    ...
    {
        return $this->queryInModules($moduleIds)
            ->with(['module', 'assignee.profile', 'createdBy.profile'])
            ->findOrFail($id);
    }
    ...
    public function datatable(array $filters, int $page = 1, int $size = 15, array|string|null $moduleIds = null): array
    {
        ...
        $query = $this->buildDatatableQuery($filters, $moduleIds);
    ...
    {
        $my = $this->queryInModules($moduleIds)
            ->where('assigned_to_user_id', $userId)
    ...
    public function adminDashboardStats(int $months = 6, array|string|null $moduleIds = null): array
    {
        ...
        $tasks = $this->queryInModules($moduleIds)
            ->select([
                'id',
                'module_id',
                'status',
        ...
        $currentPending = $this->queryInModules($moduleIds)
            ->where('status', Task::STATUS_PENDING)
            ->count();

        $currentInProgress = $this->queryInModules($moduleIds)
    ...
    private function buildDatatableQuery(array $filters, array|string|null $moduleIds = null): Builder
    {
        $actorUserId = (string) ($filters['actor_user_id'] ?? '');
        $actorRolesByModule = is_array($filters['actor_roles_by_module'] ?? null)
            ? $filters['actor_roles_by_module']
            : [];
        ...
        $moduleFilter = trim((string) ($filters['module_id'] ?? ''));

        $query = $this->queryInModules($moduleIds)->with(['assignee.profile', 'module']);
        ...
        if ($scope === 'all' && $canViewAll) {
            // show all accessible owner-module records
    ...
    private function queryInModules(array|string|null $moduleIds = null): Builder
    {
        ...
        return Task::query()
            ->when($normalizedModuleIds !== null, function (Builder $query) use ($normalizedModuleIds) {
                ...
                $query->whereIn('module_id', $normalizedModuleIds);
            });
    }

    private function applyEligibleRoleScope(Builder $query, array $rolesByModule = []): Builder
    {
        $normalizedRoleMap = $this->normalizeRoleMap($rolesByModule);

        return $query->where(function (Builder $qb) use ($normalizedRoleMap) {
            if ($normalizedRoleMap !== []) {
                foreach ($normalizedRoleMap as $moduleId => $roles) {
                    $qb->orWhere(function (Builder $moduleQuery) use ($moduleId, $roles) {
                        $moduleQuery->where('module_id', $moduleId);
                        $this->applyEligibleRolesClause($moduleQuery, $roles);
                    });
                }
            ...
            $this->applyEligibleRolesClause($qb, []);
        });
    }

    private function applyEligibleRolesClause(Builder $query, array $roles): void
    {
        $query->where(function (Builder $eligibilityQuery) use ($roles) {
            $eligibilityQuery->whereNull('data->eligible_roles');

            foreach ($roles as $role) {
                $eligibilityQuery->orWhereJsonContains('data->eligible_roles', $role);
            }
        });
    }

    private function normalizeModuleIds(array|string|null $moduleIds): ?array
    {
        if ($moduleIds === null) {
            return null;
        }

        $normalized = collect(is_array($moduleIds) ? $moduleIds : [$moduleIds])
    ...
    private function normalizeRoleMap(array $rolesByModule): array
    {
        $normalized = [];

        foreach ($rolesByModule as $moduleId => $roles) {
            $key = trim((string) $moduleId);

            if ($key === '') {
                continue;
            }

            $normalized[$key] = collect(is_array($roles) ? $roles : [$roles])
