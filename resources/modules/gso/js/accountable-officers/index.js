  ...
  async function parseErrorResponse(response) {
    ...
    return (
      ...
      (response.status === 401
        ...
        : response.status === 403
        ? "You do not have permission to manage accountable officers."
        : response.status === 419
        ? "Your security token expired. Refresh the page and try again."
        : response.status === 404
        ? "The accountable officer could not be found."
        : "The request could not be completed.")
    );
  }

  onReady(function () {
    const tableElement = document.getElementById("accountable-officers-table");
    if (!tableElement) return;

    const config = window.__gsoAccountableOfficers || {};
    const infoElement = document.getElementById("accountable-officers-info");
    const searchInput = document.getElementById("accountable-officers-search");
    const statusSelect = document.getElementById("accountable-officers-status");
    const departmentFilterSelect = document.getElementById("accountable-officers-department-filter");
    const clearButton = document.getElementById("accountable-officers-clear");

    let filters = {
      search: "",
      archived: statusSelect?.value || "active",
      department_id: departmentFilterSelect?.value || "",
    };
    let lastTotal = 0;
    ...
    const table = new Tabulator(tableElement, {
      ...
      placeholder: "No accountable officers found.",
      pagination: "remote",
      paginationSize: 15,
      ...
      columns: [
        ...
        { title: "Created", field: "created_at_text", width: 200 },
        {
          ...
          formatter: (cell) => {
            ...
            return `
              <div class="hstack flex gap-2 justify-end">
                <button class="ti-btn ti-btn-sm ti-btn-info !rounded-full" type="button" data-action="edit-accountable-officer" data-row='${escapeHtml(JSON.stringify(row))}'>
    ...
    window.__gsoAccountableOfficersReload = () => reload(table);

    const applyFilters = debounce(() => {
      filters.search = (searchInput?.value || "").trim();
      filters.archived = (statusSelect?.value || "active").trim();
      filters.department_id = (departmentFilterSelect?.value || "").trim();
      reload(table);
    });
    ...
    clearButton?.addEventListener("click", () => {
      ...
      filters = { search: "", archived: "active", department_id: "" };
      reload(table);
    });
    ...
    document.addEventListener("click", async (event) => {
      ...
      const confirmation = await Swal.fire({
        ...
        title: isRestore ? "Restore accountable officer?" : "Archive accountable officer?",
        text: isRestore
          ? "This will restore the accountable officer."
          : "This will archive the accountable officer.",
