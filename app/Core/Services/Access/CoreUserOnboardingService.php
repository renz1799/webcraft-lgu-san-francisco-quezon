...
class CoreUserOnboardingService implements CoreUserOnboardingServiceInterface
{
    ...
    {
        $modules = $this->modulesForOnboarding();
        $selectedModule = $this->defaultSelectedModule($modules);
        $selectedModuleId = (string) ($selectedModule?->id ?? '');
        $selectedDepartmentId = (string) ($selectedModuleId !== ''
            ? ($this->moduleDepartments->defaultDepartmentIdForModule($selectedModuleId) ?? '')
            : '');
        ...
        return [
            ...
            'departmentsByModule' => $this->departmentsByModule($modules),
            'rolesByModule' => $this->rolesByModule($modules),
            'moduleHints' => $this->moduleHints($modules),
        ];
    }

    public function onboard(User $actor, CoreUserOnboardingData $data): array
    {
        $module = $this->resolveModule($data->moduleId);
        $moduleId = (string) $module->id;
        $moduleName = trim((string) ($module->name ?: $module->code ?: 'Module')) ?: 'Module';
        $resolvedDepartmentId = $this->moduleDepartments->resolveForModule($moduleId, $data->departmentId);
        ...
        if ($existingUser?->trashed()) {
            throw ValidationException::withMessages([
                'email' => 'This platform account is archived in Core Platform. Restore it before onboarding.',
            ]);
        }

        if ($existingUser instanceof User && ! (bool) $existingUser->is_active) {
            throw ValidationException::withMessages([
                'email' => 'This platform account is inactive in Core Platform. Reactivate it before onboarding.',
            ]);
        }

        $result = DB::transaction(function () use ($data, $existingUser, $moduleId, $resolvedDepartmentId, $targetRole): array {
            ...
            $currentRoleName = $this->roleAssignments->rolesInModule($user, $moduleId)->first()?->name;
            ...
            if ($roleChanged) {
                $this->roleAssignments->syncInModule($user, [$targetRole], $moduleId);
            }
        ...
        $notification = $this->credentialNotifications->send(
            ...
            departmentName: $this->departmentName($resolvedDepartmentId),
    ...
    private function modulesForOnboarding(): Collection
    {
        return Module::query()
            ->where('is_active', true)
            ->orderByRaw("case when upper(code) = 'CORE' then 0 else 1 end")
            ->orderBy('name')
            ->get(['id', 'code', 'name', 'type', 'default_department_id']);
    }

    private function defaultSelectedModule(Collection $modules): ?Module
    {
        $currentModuleId = (string) ($this->context->moduleId() ?? '');

        if ($currentModuleId !== '') {
            $match = $modules->first(fn (Module $module) => (string) $module->id === $currentModuleId);

            if ($match instanceof Module) {
                return $match;
            }
        }
    ...
    private function departmentsByModule(Collection $modules): array
    {
        return $modules
            ->mapWithKeys(function (Module $module): array {
                $items = $this->moduleDepartments->allowedDepartmentsForModule((string) $module->id)
                ...
                return [(string) $module->id => $items];
            })
            ->all();
    }

    private function rolesByModule(Collection $modules): array
    {
        $moduleIds = $modules->pluck('id')->map(fn ($id) => (string) $id)->all();
        ...
        $roles = Role::query()
            ...
            ->groupBy(fn (Role $role) => (string) $role->module_id);

        return $modules
            ->mapWithKeys(function (Module $module) use ($roles): array {
                $items = collect($roles->get((string) $module->id, []))
    ...
    private function moduleHints(Collection $modules): array
    {
        return $modules
            ->mapWithKeys(function (Module $module): array {
                $defaultDepartment = $this->moduleDepartments->allowedDepartmentsForModule((string) $module->id)->first();

                return [(string) $module->id => [
                    'module_name' => trim((string) ($module->name ?: $module->code ?: 'Module')) ?: 'Module',
                    'default_department_label' => $defaultDepartment?->name ?: 'No default department configured',
                ]];
            })
            ->all();
    }
    ...
    private function resolveModule(string $moduleId): Module
    {
        ...
        if (! $module) {
            throw ValidationException::withMessages([
                'module_id' => 'Select a valid active module for onboarding.',
            ]);
        }
    ...
    private function resolveRole(string $moduleId, string $roleName): Role
    {
        ...
        if (! $role) {
            throw ValidationException::withMessages([
                'role' => 'Select a valid role for the selected module.',
            ]);
        }
    ...
    private function createPlatformIdentity(CoreUserOnboardingData $data, ?string $departmentId): User
    {
    ...
    private function recordOnboardingAudit(
        ...
        $this->audit->record(
            'user.core_onboarding.completed',
            $user,
            [
            ...
            [
                'trigger' => 'core_onboarding',
                'channel' => $credentialEmailSent ? 'email' : 'system',
                'notification_type' => $notificationType,
            ],
            'Platform user onboarded from Core Platform.',
            [
                'summary' => 'Core onboarding completed for ' . $this->userDisplayName($user),
    ...
    ): void
    {
        if ($notificationType === 'invitation') {
            ...
            $this->audit->record(
                ...
                'Invitation email sent during Core onboarding.',
                [
        ...
        $this->audit->record(
            ...
            'Module access notification email sent during Core onboarding.',
            [
    ...
    private function buildSuccessMessage(
        ...
        if ($identityCreated) {
            $message = 'Platform account created and assigned to ' . $moduleName . '.';
        } elseif ($membershipCreated) {
            $message = 'Existing platform account assigned to ' . $moduleName . '.';
        } else {
        ...
        if ($notificationSent) {
            ...
            $message .= ' The user was onboarded, but the onboarding email could not be sent automatically.';
        }
