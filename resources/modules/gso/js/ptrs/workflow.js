...
(function () {
      ...
      message:
        ...
        (res.status === 401
          ...
          : res.status === 403
            ...
            : res.status === 422
              ? "Please review the PTR details and selected items before continuing."
      ...
      await Swal.fire({
        ...
        html: `
          <div style="text-align:left">
            <div style="margin-bottom:8px;">${esc(err?.message || "Please review the current PTR details.")}</div>
  ...
  onReady(function () {
    const submitBtn = document.getElementById("ptrSubmitBtn");
    const reopenBtn = document.getElementById("ptrReopenBtn");
    const finalizeBtn = document.getElementById("ptrFinalizeBtn");
    const cancelBtn = document.getElementById("ptrCancelBtn");
    ...
    submitBtn?.addEventListener("click", async function () {
      ...
      if (itemCount <= 0) {
        await Swal.fire({
          ...
          text: "Add at least one PTR item before submitting.",
        });
        return;
      }
      ...
      const confirm = await Swal.fire({
        icon: "question",
        title: dirtyCount > 0 ? "Save and Submit PTR?" : "Submit PTR?",
        html: `
          <div style="text-align:left">
            <div>${
              dirtyCount > 0
                ? `This will save <b>${dirtyCount}</b> pending header change${dirtyCount === 1 ? "" : "s"} and then submit the PTR.`
                : "This will move the PTR forward for transfer finalization."
            }</div>
      ...
      try {
        ...
        await Swal.fire({
          icon: "success",
          title: "Submitted",
          text: dirtyCount > 0 ? "PTR was saved and submitted." : "PTR is now submitted.",
          timer: 1000,
        ...
        await showError(err, "Unable to submit PTR");
        lockBtn(submitBtn, false);
        page.refreshDirtyState?.();
      }
    });
    ...
    reopenBtn?.addEventListener("click", async function () {
      const confirm = await Swal.fire({
        ...
        html: `
          <div style="text-align:left">
            <div>This will move the PTR back to <b>DRAFT</b> so the transfer details and items can be edited again.</div>
            <div class="text-xs" style="color:#8c9097;margin-top:6px">
      ...
      try {
        ...
        await Swal.fire({
          icon: "success",
          title: "Reopened",
          text: "PTR is back in draft and can be edited again.",
          timer: 1000,
          showConfirmButton: false,
        });
        ...
        await showError(err, "Unable to reopen PTR");
    ...
    finalizeBtn?.addEventListener("click", async function () {
      const confirm = await Swal.fire({
        ...
        html: `
          <div style="text-align:left">
            <div>This will generate the PTR number and transfer the selected inventory items to the destination department, fund source, and accountable officer.</div>
            <div class="text-xs" style="color:#8c9097;margin-top:6px">
      ...
      try {
        ...
        await Swal.fire({
          icon: "success",
          title: "Finalized",
          text: "PTR is finalized and the selected inventory items were transferred.",
          timer: 1100,
          showConfirmButton: false,
        });
        ...
        await showError(err, "Unable to finalize PTR");
    ...
    cancelBtn?.addEventListener("click", async function () {
      ...
      try {
        ...
        await showError(err, "Unable to cancel PTR");
