...
(function () {
  ...
  onReady(function () {
    const tableElement = document.getElementById("departments-table");
    if (!tableElement) return;

    const config = window.__gsoDepartments || {};
    const infoElement = document.getElementById("departments-info");
    const searchInput = document.getElementById("departments-search");
    const statusSelect = document.getElementById("departments-status");
    const clearButton = document.getElementById("departments-clear");
      ...
      columns: [
        ...
        { title: "Created", field: "created_at_text", width: 200 },
        {
          ...
          formatter: (cell) => {
            ...
            return `
              <div class="hstack flex gap-2 justify-end">
                <button class="ti-btn ti-btn-sm ti-btn-info !rounded-full" type="button" data-action="edit-department" data-row='${escapeHtml(JSON.stringify(row))}'>
    ...
    window.__gsoDepartmentsReload = () => reload(table);
    ...
    document.addEventListener("click", async (event) => {
      ...
      const confirmation = await Swal.fire({
        ...
        title: isRestore ? "Restore department?" : "Archive department?",
        text: isRestore
          ? "This will restore the department."
