  ...
  async function parseErrorResponse(response) {
    ...
    return (
      ...
      (response.status === 401
        ...
        : response.status === 403
        ? "You do not have permission to manage fund clusters."
        : response.status === 419
        ? "Your security token expired. Refresh the page and try again."
        : response.status === 404
        ? "The fund cluster could not be found."
        : "The request could not be completed.")
    );
  }

  onReady(function () {
    const tableElement = document.getElementById("fund-clusters-table");
    if (!tableElement) return;

    const config = window.__gsoFundClusters || {};
    const infoElement = document.getElementById("fund-clusters-info");
    const searchInput = document.getElementById("fund-clusters-search");
    const statusSelect = document.getElementById("fund-clusters-status");
    const clearButton = document.getElementById("fund-clusters-clear");
    ...
    const table = new Tabulator(tableElement, {
      ...
      placeholder: "No fund clusters found.",
      pagination: "remote",
      paginationSize: 15,
      ...
      columns: [
        ...
        { title: "Status", field: "is_active_text", width: 150 },
        { title: "Created", field: "created_at_text", width: 200 },
        {
          ...
          formatter: (cell) => {
            ...
            return `
              <div class="hstack flex gap-2 justify-end">
                <button class="ti-btn ti-btn-sm ti-btn-info !rounded-full" type="button" data-action="edit-fund-cluster" data-row='${escapeHtml(JSON.stringify(row))}'>
    ...
    window.__gsoFundClustersReload = () => reload(table);
    ...
    document.addEventListener("click", async (event) => {
      ...
      const confirmation = await Swal.fire({
        ...
        title: isRestore ? "Restore fund cluster?" : "Archive fund cluster?",
        text: isRestore
          ? "This will restore the fund cluster."
          : "This will archive the fund cluster.",
