...
class RpcppeReportService extends AbstractPhysicalCountReportService implements RpcppeReportServiceInterface
{
    ...
    public function getPrintViewData(
        ...
        $paperProfile = $this->resolveRpcppePaperProfile($requestedPaper, $paperOverrides);
        $rows = $legacy['rows'] ?? [];
        $pagination = $this->buildRpcppePagination($rows, $paperProfile);
    ...
    public function generatePdf(
        ...
        $filename = 'rpcppe-report-' . now()->format('Ymd-His') . '-' . Str::uuid() . '.pdf';
        $outputPath = storage_path('app/tmp/' . $filename);

        return $this->pdfGenerator->generateFromView(
            view: 'gso::reports.rpcppe.print.pdf',
            data: [
     ...
     */
    private function buildRpcppePagination(array $rows, array $paperProfile): array
    {
        ...
        if ($rows === []) {
            ...
            while ($cursor < count($rows)) {
                ...
                while ($cursor < count($rows)) {
                    $row = $rows[$cursor];
                    $rowUnits = $this->estimateRpcppeRowUnits($row, $descriptionCharsPerLine);
                ...
                if ($pageRows === []) {
                    ...
                    $usedUnits = $this->estimateRpcppeRowUnits($rows[$cursor], $descriptionCharsPerLine);
                    $cursor++;
                }
     ...
     */
    private function estimateRpcppeRowUnits(array $row, int $descriptionCharsPerLine): int
    {
        $maxUnits = max(
            $this->estimateRpcppeCellUnits($row['article'] ?? '', max(12, (int) round($descriptionCharsPerLine * 0.55))),
            $this->estimateRpcppeCellUnits($row['description'] ?? '', $descriptionCharsPerLine),
            $this->estimateRpcppeCellUnits($row['property_no'] ?? '', max(10, (int) round($descriptionCharsPerLine * 0.45))),
            $this->estimateRpcppeCellUnits($row['remarks'] ?? '', max(10, (int) round($descriptionCharsPerLine * 0.45))),
        );

        return max(1, $maxUnits);
    }

    private function estimateRpcppeCellUnits(mixed $value, int $charsPerLine): int
    {
     ...
     * @return array<string, mixed>
     */
    private function resolveRpcppePaperProfile(?string $requestedPaper, array $overrides): array
    {
        $paperProfile = $this->printConfigLoader->resolvePaperProfile('gso_rpcppe', $requestedPaper);
