...
<head>
  ...
  <title>RPCSP - {{ $report['as_of_label'] ?? 'Report' }}</title>
...
>
  <x-slot:sidebar>
    <x-print.panel
      kicker="Reports"
      title="RPCSP Preview"
      copy="Set the cutoff, office scope, and signatories here, then review the printable semi-expendable count report on the right."
    >
      ...
      <form method="GET" action="{{ route('gso.inventory-items.rpcsp.print') }}" class="gso-report-filter-form">
        <input type="hidden" name="preview" value="1">
        <input type="hidden" name="prefill_count" value="0">

        <div class="gso-report-filter-group">
          <label for="rpcsp-fund-source">Fund Source</label>
          <select id="rpcsp-fund-source" name="fund_source_id">
        ...
        <div class="gso-report-filter-group">
          <label for="rpcsp-office">Office</label>
          <select id="rpcsp-office" name="department_id">
        ...
        <div class="gso-report-filter-group">
          <label for="rpcsp-officer">Accountable Officer</label>
          <select id="rpcsp-officer" name="accountable_officer_id">
        ...
        <div class="gso-report-filter-group">
          <label for="rpcsp-as-of">As of Date</label>
          <input id="rpcsp-as-of" type="date" name="as_of" value="{{ $report['as_of'] ?? '' }}">
        </div>

        <label class="gso-report-toggle">
          <input type="checkbox" name="prefill_count" value="1" @checked(!empty($report['prefill_count']))>
          <span>Prefill the physical-count and shortage or overage columns from the book quantity.</span>
        </label>

        <div class="gso-report-filter-group">
          <label for="rpcsp-accountable-name">Accountable Officer Signatory</label>
          <input id="rpcsp-accountable-name" type="text" name="accountable_officer_name" value="{{ $signatories['accountable_officer_name'] ?? '' }}">
        </div>

        <div class="gso-report-filter-group">
          <label for="rpcsp-accountable-designation">Accountable Designation</label>
          <input id="rpcsp-accountable-designation" type="text" name="accountable_officer_designation" value="{{ $signatories['accountable_officer_designation'] ?? '' }}">
        </div>

        <div class="gso-report-filter-group">
          <label for="rpcsp-chair">Inventory Committee Chair</label>
          <input id="rpcsp-chair" type="text" name="committee_chair_name" value="{{ $signatories['committee_chair_name'] ?? '' }}">
        </div>

        <div class="gso-report-filter-group">
          <label for="rpcsp-member-1">Committee Member 1</label>
          <input id="rpcsp-member-1" type="text" name="committee_member_1_name" value="{{ $signatories['committee_member_1_name'] ?? '' }}">
        </div>

        <div class="gso-report-filter-group">
          <label for="rpcsp-member-2">Committee Member 2</label>
          <input id="rpcsp-member-2" type="text" name="committee_member_2_name" value="{{ $signatories['committee_member_2_name'] ?? '' }}">
        </div>

        <div class="gso-report-filter-group">
          <label for="rpcsp-approved-name">Approved By</label>
          <input id="rpcsp-approved-name" type="text" name="approved_by_name" value="{{ $signatories['approved_by_name'] ?? '' }}">
        </div>

        <div class="gso-report-filter-group">
          <label for="rpcsp-approved-designation">Approved Designation</label>
          <input id="rpcsp-approved-designation" type="text" name="approved_by_designation" value="{{ $signatories['approved_by_designation'] ?? '' }}">
        </div>

        <div class="gso-report-filter-group">
          <label for="rpcsp-verified-name">Verified By</label>
          <input id="rpcsp-verified-name" type="text" name="verified_by_name" value="{{ $signatories['verified_by_name'] ?? '' }}">
        </div>

        <div class="gso-report-filter-group">
          <label for="rpcsp-verified-designation">Verified Designation</label>
          <input id="rpcsp-verified-designation" type="text" name="verified_by_designation" value="{{ $signatories['verified_by_designation'] ?? '' }}">
        </div>
  ...
  @foreach($pages as $pageIndex => $pageRows)
    ...
    <div class="gso-report-page print-page">
      <div class="gso-report-content">
        <div class="gso-report-appendix">{{ $report['appendix_label'] ?? 'RPCSP' }}</div>
        <div class="gso-report-title">Report on the Physical Count of Semi-Expendable Property</div>
        ...
        <table class="gso-report-meta-table">
          <tr>
            ...
            <td style="width:50%;"><span class="gso-report-meta-label">Fund Cluster:</span> {{ $report['fund_cluster'] ?? 'All / Multiple' }}</td>
          </tr>
          <tr>
            <td><span class="gso-report-meta-label">Fund Source:</span> {{ $report['fund_source'] ?? 'All Fund Sources' }}</td>
            <td><span class="gso-report-meta-label">As of:</span> {{ $report['as_of_label'] ?? '' }}</td>
          </tr>
          <tr>
            <td><span class="gso-report-meta-label">Office:</span> {{ $report['department'] ?? 'All Offices' }}</td>
            <td><span class="gso-report-meta-label">Accountable Officer:</span> {{ $report['accountable_officer'] ?? 'All Accountable Officers' }}</td>
          </tr>
          <tr>
            <td colspan="2">
        ...
        <table class="gso-report-items-table">
          <colgroup>
            <col style="width:12%;"><col style="width:20%;"><col style="width:12%;"><col style="width:6%;"><col style="width:8%;">
            <col style="width:9%;"><col style="width:9%;"><col style="width:7%;"><col style="width:8%;"><col style="width:9%;">
          </colgroup>
          <thead>
            <tr>
            ...
            <tr><th>Qty.</th><th>Value</th></tr>
          </thead>
          <tbody>
            @if($pageRows->isEmpty())
              <tr><td colspan="10" class="gso-report-empty-note">No semi-expendable balances found for the selected report window.</td></tr>
        ...
        @if($isLastPage)
          <table class="gso-report-summary-table">
            <tr><th>Items Listed</th><th>Qty. per Card</th><th>Qty. per Count</th><th>Shortage / Overage Qty.</th><th>Book Value</th></tr>
            <tr>
          ...
          <table class="gso-report-signature-table">
            <tr><th>Accountable Officer</th><th>Inventory Committee</th><th>Approved By</th><th>Verified By</th></tr>
            <tr>
              <td>
                <div class="gso-report-signature-name">{{ $signatories['accountable_officer_name'] ?? '' }}</div>
                <div class="gso-report-signature-role">{{ $signatories['accountable_officer_designation'] ?? '' }}</div>
              </td>
              <td>
