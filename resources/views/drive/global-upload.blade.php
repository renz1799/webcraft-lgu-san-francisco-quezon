...
<div class="container mx-auto max-w-xl">
    <h2 class="text-lg font-semibold mb-4">Google Drive – Global Upload Test</h2>
    ...
    <form method="POST" action="{{ route('drive.global.connect') }}" class="mb-6">
        @csrf

        @if(!$connected)
            @if(auth()->user()->hasRole('Administrator') || auth()->user()->can('modify Google Drive Connection'))
                <form method="POST" action="{{ route('drive.global.connect') }}" class="mb-6">
                    @csrf
            ...
            @else
                <div class="p-3 mb-4 border rounded text-warning border-warning">
                    Google Drive is not connected. Please ask an administrator to connect it.
                </div>
            @endif
        @else
            <div class="p-3 mb-4 border rounded text-success border-success">
        ...
        @if($isImage)
            <div class="mt-4">
                ...
                <img
                    src="{{ route('drive.global.preview', $uploaded['drive_file_id']) }}"
    ...
    <form method="POST" action="{{ route('drive.global.upload') }}" enctype="multipart/form-data" class="space-y-4 border p-4 rounded">
        @csrf
