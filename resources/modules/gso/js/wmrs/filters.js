(function () {
  ...
  if (window.__wmrFiltersBound) return;
  window.__wmrFiltersBound = true;
  ...
  onReady(function () {
    const search = document.getElementById("wmr-search");
    const clear = document.getElementById("wmr-clear");
    const workflowStatusSel = document.getElementById("wmr-status");
    const moreBtn = document.getElementById("wmr-more-btn");
    const morePanel = document.getElementById("wmr-more-panel");
    const moreClose = document.getElementById("wmr-more-close");
    const dateFrom = document.getElementById("wmr-date-from");
    const dateTo = document.getElementById("wmr-date-to");
    const fundClusterSel = document.getElementById("wmr-fund-cluster");
    const recordStatusSel = document.getElementById("wmr-record-status");
    const advApply = document.getElementById("wmr-adv-apply");
    const advReset = document.getElementById("wmr-adv-reset");
    const advCount = document.getElementById("wmr-adv-count");
    ...
    function syncFromUI() {
      ...
      filters.fund_cluster_id = (fundClusterSel?.value || "").trim();
    ...
    function countAdvanced() {
      ...
      if ((fundClusterSel?.value || "").trim() !== "") n++;
    ...
    if (typeof window.__wmrReload === "function") {
      reloadFn = () => window.__wmrReload();
    } else {
      let tries = 0;
      const maxTries = 80;
      const timer = setInterval(() => {
        ...
        if (window.__wmrTable && typeof window.__wmrTable.setData === "function") {
          clearInterval(timer);
          reloadFn = () => window.__wmrTable.setData();
          return;
        }
    ...
    function portalToBody() {
      if (portaled) return;
      placeholder = document.createComment("wmr-more-panel-placeholder");
    ...
    advReset?.addEventListener("click", (e) => {
      ...
      if (fundClusterSel) fundClusterSel.value = "";
