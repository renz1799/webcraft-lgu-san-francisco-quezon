...
class AirPrintService implements AirPrintServiceInterface
{
    ...
    private const RAW_ROWS_PER_PAGE = 22;
    ...
    public function getPrintViewData(string $airId): array
    {
        ...
        $pages = $this->paginateRows($rows);

        return [
            'air' => $this->buildAirSummary($air),
            'print' => $this->buildPrintMeta($air, $rows, count($pages)),
            'rows' => $rows,
            'pages' => $pages,
            'totalPages' => count($pages),
            'maxGridRows' => self::MAX_GRID_ROWS,
        ];
    }
    ...
    private function paginateRows(array $rows): array
    {
        ...
        $rawPages = array_chunk($rows, self::RAW_ROWS_PER_PAGE);
        $totalPages = count($rawPages);
        $pages = [];

        foreach ($rawPages as $index => $pageRows) {
            $pageNo = $index + 1;

            if ($totalPages > 1 && $pageNo > 1) {
                array_unshift(
                    $pageRows,
                    $this->messageRow('*** CONTINUATION FROM PAGE ' . ($pageNo - 1) . ' ***')
                );
            }

            if ($totalPages > 1 && $pageNo < $totalPages) {
                $pageRows[] = $this->messageRow('*** CONTINUED ON PAGE ' . ($pageNo + 1) . ' ***');
            }
     ...
     */
    private function buildPrintMeta(Air $air, array $rows, int $pageCount): array
    {
     ...
     * @return array<string, mixed>
     */
    private function messageRow(string $message): array
    {
        return [
            'property_no' => '',
            'description' => $message,
