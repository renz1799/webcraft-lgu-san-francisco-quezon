...
(function () {
  ...
  function collectPayload() {
    ...
    if (payload.transfer_type !== "others") {
  ...
  function syncTransferTypeUi() {
    const select = document.getElementById("ptrTransferTypeSelect");
    const other = document.getElementById("ptrTransferTypeOther");
  ...
  onReady(function () {
    ...
    const saveBtn = document.getElementById("ptrSaveBtn");
    ...
    const fromOfficerInput = document.getElementById("ptrFromAccountableOfficerInput");
    const toOfficerInput = document.getElementById("ptrToAccountableOfficerInput");
    const approvedByInput = document.getElementById("ptrApprovedByNameInput") || document.querySelector('[name="approved_by_name"]');
    const approvedByDesignationField = document.getElementById("ptrApprovedByDesignationInput") || document.querySelector('[name="approved_by_designation"]');
    const releasedByInput = document.getElementById("ptrReleasedByNameInput") || document.querySelector('[name="released_by_name"]');
    const releasedByDesignationField = document.getElementById("ptrReleasedByDesignationInput") || document.querySelector('[name="released_by_designation"]');
    const receivedByInput = document.getElementById("ptrReceivedByNameInput") || document.querySelector('[name="received_by_name"]');
    const receivedByDesignationField = document.getElementById("ptrReceivedByDesignationInput") || document.querySelector('[name="received_by_designation"]');
    ...
    async function saveChanges(options = {}) {
      ...
      if (dirtyFields.length <= 0) {
        if (!silentNoChanges) {
          await Swal.fire({
            ...
            text: "The PTR header is already up to date.",
          });
        }
        return true;
      }
      ...
      try {
        const res = await fetch(window.__ptrEdit.updateUrl, {
          method: "PUT",
          headers: {
        ...
        syncFundClusterDisplay("ptrFromFundSourceSelect", "ptrFromFundClusterDisplay");
        syncFundClusterDisplay("ptrToFundSourceSelect", "ptrToFundClusterDisplay");
        syncTransferTypeUi();
        updateActionUi();
        ...
        if (!silentSuccess) {
          await Swal.fire({
            icon: "success",
            title: "Saved",
            text: data?.message || "PTR draft updated successfully.",
            timer: 1200,
    ...
    async function saveOfficerRecord(payload) {
      const response = await fetch(window.__ptrEdit.accountableOfficerStoreUrl, {
        method: "POST",
        headers: {
          "X-CSRF-TOKEN": getCsrf(),
          Accept: "application/json",
    ...
    async function openOfficerDetailsModal({
      ...
      if (!departmentField) {
        await Swal.fire({
          ...
          text: "PTR could not resolve the department options needed for this accountable officer.",
        });
        return null;
      }
        ...
        html: `
          <div class="text-left space-y-3">
            <div>
              ...
              <input id="ptrOfficerModalName" class="ti-form-input w-full" value="${esc(initialOfficer.full_name || "")}" />
            </div>
            <div>
              <label class="ti-form-label !mb-1">Department <span class="text-danger">*</span></label>
              <select id="ptrOfficerModalDepartment" class="ti-form-select w-full">
            ...
            <div>
              ...
              <input id="ptrOfficerModalDesignation" class="ti-form-input w-full" value="${esc(initialOfficer.designation || "")}" />
            </div>
          </div>
        `,
        preConfirm: async () => {
          const selectedDepartmentId = String(document.getElementById("ptrOfficerModalDepartment")?.value || "").trim();
          const payload = {
            full_name: String(document.getElementById("ptrOfficerModalName")?.value || "").trim(),
            department_id: selectedDepartmentId,
            designation: String(document.getElementById("ptrOfficerModalDesignation")?.value || "").trim() || null,
    ...
    async function resolveOfficerForDepartment(officer, departmentField, roleLabel) {
      ...
      const officerDepartmentName = String(resolvedOfficer.department_name || resolvedOfficer.department_label || getSelectedOptionText(departmentField, officerDepartmentId) || "that department");
      ...
      const confirm = await Swal.fire({
        ...
        html: `
          <div style="text-align:left">
            ...
            <p style="margin:0;">The PTR currently points to <b>${esc(currentDepartmentName || "another department")}</b>. Use the officer's department instead?</p>
          </div>
        `,
        showCancelButton: true,
    ...
    document.querySelectorAll(inputSelector).forEach((el) => {
      ...
      el.addEventListener("change", () => {
        if (el.name === "from_fund_source_id") {
          syncFundClusterDisplay("ptrFromFundSourceSelect", "ptrFromFundClusterDisplay");
        }
        if (el.name === "to_fund_source_id") {
          syncFundClusterDisplay("ptrToFundSourceSelect", "ptrToFundClusterDisplay");
        }
        if (el.name === "transfer_type") {
          syncTransferTypeUi();
        }
        updateActionUi();
      });
    ...
    const fromOfficerHandle = attachAccountableOfficerAutocomplete({
      input: "#ptrFromAccountableOfficerInput",
      suggestUrl: window.__ptrEdit?.accountableOfficerSuggestUrl,
      storeUrl: window.__ptrEdit?.accountableOfficerStoreUrl,
      ...
      createOfficer: async ({ name }) => {
        return await openOfficerDetailsModal({
          title: "Create Source Accountable Officer",
          confirmButtonText: "Create and Use",
          departmentField: fromDepartmentField,
          initialOfficer: { full_name: name },
        });
      },
      beforeApplyOfficer: async (officer) => {
        return await resolveOfficerForDepartment(officer, fromDepartmentField, "Source Accountable Officer");
      },
    });

    const toOfficerHandle = attachAccountableOfficerAutocomplete({
      input: "#ptrToAccountableOfficerInput",
      suggestUrl: window.__ptrEdit?.accountableOfficerSuggestUrl,
      storeUrl: window.__ptrEdit?.accountableOfficerStoreUrl,
      ...
      createOfficer: async ({ name }) => {
        return await openOfficerDetailsModal({
          title: "Create Destination Accountable Officer",
          confirmButtonText: "Create and Use",
          departmentField: toDepartmentField,
          initialOfficer: { full_name: name },
        });
      },
      beforeApplyOfficer: async (officer) => {
        return await resolveOfficerForDepartment(officer, toDepartmentField, "Destination Accountable Officer");
      },
    });

    attachDepartmentGuard({
      departmentField: fromDepartmentField,
      officerInput: fromOfficerInput,
      handle: fromOfficerHandle,
      roleLabel: "From Accountable Officer",
    });

    attachDepartmentGuard({
      departmentField: toDepartmentField,
      officerInput: toOfficerInput,
      handle: toOfficerHandle,
      roleLabel: "To Accountable Officer",
    });

    attachAccountableOfficerAutocomplete({
      input: "#ptrApprovedByNameInput",
      suggestUrl: window.__ptrEdit?.accountableOfficerSuggestUrl,
      storeUrl: window.__ptrEdit?.accountableOfficerStoreUrl,
      departmentField: '[name="from_department_id"]',
      ...
      createOfficer: async ({ name }) => {
        return await openOfficerDetailsModal({
          title: "Create Approving Officer",
          confirmButtonText: "Create and Use",
          departmentField: fromDepartmentField,
          initialOfficer: { full_name: name },
          initialDepartmentId: String(fromDepartmentField?.value || "").trim(),
        });
      },
      beforeApplyOfficer: async (officer) => {
        return await resolveOfficerDetailsOnly(officer, {
          title: "Complete Approving Officer Details",
          departmentField: fromDepartmentField,
          initialDepartmentId: String(fromDepartmentField?.value || "").trim(),
        });
      },
    ...
    attachAccountableOfficerAutocomplete({
      ...
      createOfficer: async ({ name }) => {
        return await openOfficerDetailsModal({
          title: "Create Released / Issued By Officer",
          confirmButtonText: "Create and Use",
          departmentField: fromDepartmentField,
          initialOfficer: { full_name: name },
          initialDepartmentId: String(fromDepartmentField?.value || "").trim(),
      ...
      beforeApplyOfficer: async (officer) => {
        return await resolveOfficerDetailsOnly(officer, {
          title: "Complete Released / Issued By Details",
          departmentField: fromDepartmentField,
          initialDepartmentId: String(fromDepartmentField?.value || "").trim(),
        });
      },
    ...
    attachAccountableOfficerAutocomplete({
      ...
      createOfficer: async ({ name }) => {
        return await openOfficerDetailsModal({
          title: "Create Receiving Officer",
          confirmButtonText: "Create and Use",
          departmentField: toDepartmentField,
          initialOfficer: { full_name: name },
          initialDepartmentId: String(toDepartmentField?.value || "").trim(),
        });
      },
      beforeApplyOfficer: async (officer) => {
        return await resolveOfficerDetailsOnly(officer, {
          title: "Complete Receiving Officer Details",
          departmentField: toDepartmentField,
          initialDepartmentId: String(toDepartmentField?.value || "").trim(),
        });
      },
    ...
    window.__ptrEditPage = {
      ...
      getItemCount() {
        return Number(window.__ptrEdit?.initialItemCount || 0);
      },
      refreshDirtyState() {
        baseline = collectPayload();
        syncFundClusterDisplay("ptrFromFundSourceSelect", "ptrFromFundClusterDisplay");
