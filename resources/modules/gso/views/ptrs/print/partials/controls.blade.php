@php
    ...
    $printable = $printConfig->printable('gso_ptr');
    $ptr = $report['ptr'] ?? [];
    $document = $report['document'] ?? [];
    $summary = $document['summary'] ?? [];
    $pagination = $report['pagination']['stats'] ?? [];
    $selectedPaper = $filters['paper_profile'] ?? ($paperProfile['code'] ?? $printConfig->defaultPaper('gso_ptr', 'a4-portrait'));
    $allowedPapers = $printConfig->allowedPapers('gso_ptr', 'a4-portrait');
    ...
    $paperDefaults = collect($allowedPapers)
        ...
        ->all();
    $currentRowsPerPage = (int) ($filters['rows_per_page'] ?? ($paperProfile['rows_per_page'] ?? 12));
    $currentGridRows = (int) ($filters['grid_rows'] ?? ($paperProfile['grid_rows'] ?? 12));
    $currentLastPageGridRows = (int) ($filters['last_page_grid_rows'] ?? ($paperProfile['last_page_grid_rows'] ?? 12));
    $currentDescriptionCharsPerLine = (int) ($filters['description_chars_per_line'] ?? ($paperProfile['description_chars_per_line'] ?? 40));
    $layoutHelp = [
        ...
        'last_page_grid_rows' => 'Target grid height for the last page before the reason and signature section; blank rows are added until it is reached.',
    ...
    $archiveDocumentNumber = trim((string) ($document['ptr_number'] ?? ''));
@endphp

<x-print.panel
    kicker="PTR"
    title="Property Transfer Report"
    copy="Choose the paper profile, review the PTR preview on the right, then print or download the PDF."
>
    <div class="core-print-sidebar">
        <div class="core-print-sidebar__intro">
            ...
            <p class="core-print-sidebar__help">
                PTR print preview is rendered from the saved record and follows the selected paper profile.
            </p>
        </div>

        @include('gso::print.partials.archive-feedback', ['documentType' => 'PTR'])

        <form
            method="GET"
            action="{{ route('gso.ptrs.print', ['ptr' => $ptr['id'] ?? '']) }}"
            class="core-print-sidebar__form"
            data-ptr-print-form="1"
            data-ptr-print-paper-defaults='@json($paperDefaults)'
        >
                ...
                <div class="core-print-sidebar__field-grid">
                    <div class="core-print-sidebar__field">
                        ...
                        <input
                            ...
                            class="form-control"
                            data-ptr-print-setting="rows_per_page"
                        >
                    </div>
                    ...
                    <div class="core-print-sidebar__field">
                        ...
                        <input
                            ...
                            class="form-control"
                            data-ptr-print-setting="grid_rows"
                        >
                    </div>
                    ...
                    <div class="core-print-sidebar__field">
                        ...
                        <input
                            ...
                            class="form-control"
                            data-ptr-print-setting="last_page_grid_rows"
                        >
                    </div>
                    ...
                    <div class="core-print-sidebar__field">
                        ...
                        <input
                            ...
                            class="form-control"
                            data-ptr-print-setting="description_chars_per_line"
                        >
                    </div>
                </div>

                <button type="button" class="core-print-sidebar__link-button" data-ptr-print-apply-defaults="1">
            ...
            <div class="core-print-sidebar__section core-print-sidebar__section--actions">
                ...
                <div class="core-print-sidebar__actions">
                    ...
                    <a
                        href="{{ route('gso.ptrs.print.pdf', ['ptr' => $ptr['id'] ?? ''] + $pdfParams) }}"
                        data-ptr-print-pdf-download="1"
                        data-ptr-print-pdf-base="{{ route('gso.ptrs.print.pdf', ['ptr' => $ptr['id'] ?? '']) }}"
                    ...
                    @include('gso::print.partials.archive-action', [
                        ...
                        'uploadPermission' => 'ptr.upload_signed_pdf',
                    ...
                    <a href="{{ route('gso.ptrs.print', ['ptr' => $ptr['id'] ?? '']) }}" class="core-print-sidebar__reset">
                        Reset
                    </a>
                </div>
            </div>
