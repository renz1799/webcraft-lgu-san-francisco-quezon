@php
    ...
    $printable = $printConfig->printable('gso_par');
    $par = $report['par'] ?? [];
    $document = $report['document'] ?? [];
    $summary = $document['summary'] ?? [];
    $pagination = $report['pagination']['stats'] ?? [];
    $selectedPaper = $filters['paper_profile'] ?? ($paperProfile['code'] ?? $printConfig->defaultPaper('gso_par', 'a4-portrait'));
    $allowedPapers = $printConfig->allowedPapers('gso_par', 'a4-portrait');
    ...
    $paperDefaults = collect($allowedPapers)
        ...
        ->all();
    $currentRowsPerPage = (int) ($filters['rows_per_page'] ?? ($paperProfile['rows_per_page'] ?? 26));
    $currentGridRows = (int) ($filters['grid_rows'] ?? ($paperProfile['grid_rows'] ?? 26));
    $currentLastPageGridRows = (int) ($filters['last_page_grid_rows'] ?? ($paperProfile['last_page_grid_rows'] ?? 26));
    ...
    $layoutHelp = [
        ...
        'last_page_grid_rows' => 'Target grid height for the last page before the signature section; blank rows are added until it is reached.',
    ...
    $archiveDocumentNumber = trim((string) ($document['par_number'] ?? ''));
@endphp

<x-print.panel
    kicker="PAR"
    title="Property Acknowledgement Receipt"
    copy="Choose the paper profile, review the PAR preview on the right, then print or download the PDF."
>
    <div class="core-print-sidebar">
        <div class="core-print-sidebar__intro">
            ...
            <p class="core-print-sidebar__help">
                PAR print preview is rendered from the saved record and follows the selected paper profile.
            </p>
        </div>

        @include('gso::print.partials.archive-feedback', ['documentType' => 'PAR'])

        <form
            method="GET"
            action="{{ route('gso.pars.print', ['par' => $par['id'] ?? '']) }}"
            class="core-print-sidebar__form"
            data-par-print-form="1"
            data-par-print-paper-defaults='@json($paperDefaults)'
        >
                ...
                <div class="core-print-sidebar__field-grid">
                    <div class="core-print-sidebar__field">
                        ...
                        <input
                            ...
                            class="form-control"
                            data-par-print-setting="rows_per_page"
                        >
                    </div>
                    ...
                    <div class="core-print-sidebar__field">
                        ...
                        <input
                            ...
                            class="form-control"
                            data-par-print-setting="grid_rows"
                        >
                    </div>
                    ...
                    <div class="core-print-sidebar__field">
                        ...
                        <input
                            ...
                            class="form-control"
                            data-par-print-setting="last_page_grid_rows"
                        >
                    </div>
                    ...
                    <div class="core-print-sidebar__field">
                        ...
                        <input
                            ...
                            class="form-control"
                            data-par-print-setting="description_chars_per_line"
                        >
                    </div>
                </div>

                <button type="button" class="core-print-sidebar__link-button" data-par-print-apply-defaults="1">
            ...
            <div class="core-print-sidebar__section core-print-sidebar__section--actions">
                ...
                <div class="core-print-sidebar__actions">
                    ...
                    <a
                        href="{{ route('gso.pars.print.pdf', ['par' => $par['id'] ?? ''] + $pdfParams) }}"
                        data-par-print-pdf-download="1"
                        data-par-print-pdf-base="{{ route('gso.pars.print.pdf', ['par' => $par['id'] ?? '']) }}"
                    ...
                    <a href="{{ route('gso.pars.print', ['par' => $par['id'] ?? '']) }}" class="core-print-sidebar__reset">
                        Reset
                    </a>
                </div>
            </div>
