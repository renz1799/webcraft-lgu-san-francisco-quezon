...
(function () {
  ...
  function syncSourceLocks(count) {
    ...
    const help = document.getElementById("ptrItemsHelp");
    const sourceFields = [
    ...
    if (help) {
      help.textContent = count > 0
        ? "Remove all PTR items first to change the source department, source officer, or source fund source."
  ...
  function renderList(items, listWrap, emptyEl, countEl) {
    ...
    if (listWrap) {
      ...
      listWrap.innerHTML = rows.map((item) => `
        <div class="rounded border border-defaultborder p-3" data-ptr-item-row data-id="${esc(item.id)}">
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0">
              <div class="text-xs text-[#8c9097]">
                Property No.: <b>${esc(item.property_number || "-")}</b>
              </div>
  ...
  function renderSuggestions(items, listEl, suggestWrap) {
    ...
    const rows = Array.isArray(items) ? items : [];
    if (!rows.length) {
      listEl.innerHTML = '<div class="p-3 text-sm text-[#8c9097]">No matching transferable PPE items found.</div>';
    ...
    listEl.innerHTML = rows.map((item) => `
      ...
      >
        <div class="text-sm font-semibold truncate">${esc(item.property_number || "-")} - ${esc(item.item_name || "-")}</div>
  ...
  onReady(function () {
    const listWrap = document.getElementById("ptrItemsList");
    const emptyEl = document.getElementById("ptrItemsEmpty");
    const countEl = document.getElementById("ptrItemsCount");
    const search = document.getElementById("ptrItemSearch");
    const suggestWrap = document.getElementById("ptrItemSuggestions");
    const suggestList = document.getElementById("ptrItemSuggestList");
    const suggestClose = document.getElementById("ptrItemSuggestClose");
    ...
    const pageApi = window.__ptrEditPage || {};
    ...
    window.__ptrEditPage = pageApi;

    refreshList().catch((e) => {
      listWrap.classList.remove("hidden");
      listWrap.innerHTML = `<div class="text-xs text-danger">${esc(e?.message || "Unable to load PTR items.")}</div>`;
    });
    ...
    function isSourceSideDirty() {
      const page = window.__ptrEditPage;
      return !!(
        page &&
    ...
    function ensureItemActionsAllowed() {
      if (!hasSavedSourceSide()) {
        throw new Error("Save the source department and source fund source first before managing PTR items.");
      }

      if (isSourceSideDirty()) {
        throw new Error("Save the source-side changes first before managing PTR items.");
      }
    }
    ...
    listWrap.addEventListener("click", async (e) => {
      ...
      const row = btn.closest("[data-ptr-item-row]");
      ...
      const confirm = await Swal.fire({
        icon: "warning",
        title: "Remove item?",
        text: "This will remove the item from this PTR draft.",
      ...
      try {
        const url = String(cfg.itemDeleteUrlTemplate || "").replace("__PTR_ITEM_ID__", encodeURIComponent(id));
