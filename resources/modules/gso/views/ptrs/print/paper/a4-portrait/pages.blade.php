@php
    ...
    $gridRows = max(1, (int) ($paperProfile['grid_rows'] ?? 12));
    $lastPageGridRows = max(0, (int) ($paperProfile['last_page_grid_rows'] ?? 0));
    $totalPages = count($pages);
@endphp

@foreach ($pages as $pageIndex => $page)
    <div class="gso-ptr-print-page">
        @include('gso::ptrs.print.partials.header', [
            'report' => $report,
            'headerImage' => $headerImage,
            'continuationFromPage' => $totalPages > 1 && $pageIndex > 0 ? $pageIndex : null,
        ])

        <div class="gso-ptr-print-body">
            @include('gso::ptrs.print.partials.meta', [
                'report' => $report,
            ])

            @include('gso::ptrs.print.partials.table', [
                'rows' => $page['rows'] ?? [],
                'gridRows' => $gridRows,
            ...
            @if ($pageIndex === ($totalPages - 1))
                @include('gso::ptrs.print.partials.reason', [
                    'report' => $report,
                ])

                @include('gso::ptrs.print.partials.signatures', [
                    'report' => $report,
                ])
            @endif
        </div>

        @include('gso::ptrs.print.partials.footer', [
            'pageIndex' => $pageIndex,
