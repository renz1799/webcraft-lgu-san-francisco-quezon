...
class TaskService implements TaskServiceInterface
{
    ...
    public function changeStatus(
        ...
        return DB::transaction(function () use ($actorUserId, $taskId, $toStatus, $note) {
            $task = $this->findAnyTaskOrFail($taskId);
    ...
    public function reassign(
        ...
        return DB::transaction(function () use (
            ...
            $task = $this->findAnyTaskOrFail($taskId);
            $taskModuleId = trim((string) ($task->module_id ?? ''));

            $fromUser = $this->findUserInModule($task->assigned_to_user_id, $taskModuleId);
            $toUser = $this->findActiveUserInModuleOrFail($newAssigneeUserId, $taskModuleId);
    ...
    public function claim(string $actorUserId, string $taskId, ?string $note = null): Task
    {
        return DB::transaction(function () use ($actorUserId, $taskId, $note) {
            ...
            $this->findActiveUserInModuleOrFail($actorUserId, $taskModuleId);
    ...
    public function restore(string $actorUserId, string $taskId): bool
    {
        return DB::transaction(function () use ($actorUserId, $taskId) {
            $task = $this->findAnyTaskOrFailWithTrashed($taskId);
    ...
    private function findAnyTaskOrFailWithTrashed(string $taskId): Task
    {
        return $this->tasks->findOrFailWithTrashed($taskId);
    }
    ...
    private function findUserInModule(?string $userId, string $moduleId): ?User
    {
        $userId = trim((string) $userId);
        ...
        return $this->users->findInModule($userId, $moduleId);
    }
    ...
    private function findActiveUserInModuleOrFail(string $userId, string $moduleId): User
    {
        $moduleId = trim($moduleId);
        $user = $moduleId !== ''
            ? $this->users->findActiveInModule($userId, $moduleId)
            : null;
        ...
        throw new InvalidArgumentException('Assignee is not available in the task owner module.');
    }
}
