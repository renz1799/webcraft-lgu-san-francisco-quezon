...
@php
    ...
    $pdfPreviewUrl = route('gso.stocks.rpci.print.pdf', array_merge($filters ?? [], ['inline' => 1]))
        . '#' . $pdfPreviewFragment;
@endphp
...
@section('styles')
    ...
    @if ($resolvedPreviewEngine === 'dompdf')
        <style>
            .gso-rpci-pdf-preview-shell {
                width: {{ $paperProfile['width'] ?? '279.4mm' }};
                max-width: 100%;
                margin: 0 auto;
                background: #fff;
            ...
            .gso-rpci-pdf-preview-frame {
                display: block;
                width: 100%;
                height: calc({{ $paperProfile['height'] ?? '215.9mm' }} + 8mm);
                border: 0;
                background: #fff;
            }
        </style>
...
@section('content')
    ...
    <div class="print-workspace-body print-workspace-body--embedded">
        ...
        >
            ...
            @if ($resolvedPreviewEngine === 'dompdf')
                <div class="gso-rpci-pdf-preview-shell">
                    <iframe
                        class="gso-rpci-pdf-preview-frame"
                        src="{{ $pdfPreviewUrl }}"
                        title="RPCI PDF Preview"
                    ></iframe>
                </div>
            @else
